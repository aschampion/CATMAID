...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding model 'SkeletonSummary'
        # Manually create with SQL because django infers the wrong column type
        # for OneToOneFields to bigserial/bigint
        db.execute('''
            CREATE TABLE skeleton_summary (
                    skeleton_id bigint NOT NULL,
                    treenode_count bigint NOT NULL,
                    CONSTRAINT skeleton_summary_pkey PRIMARY KEY (skeleton_id),
                    CONSTRAINT skeleton_summary_skeleton_id_fkey FOREIGN KEY (skeleton_id)
                        REFERENCES class_instance (id) MATCH SIMPLE
                        ON UPDATE NO ACTION ON DELETE NO ACTION DEFERRABLE INITIALLY DEFERRED
                ) WITH (
                  OIDS=FALSE
                );
            ''')
        # Insert current treenode counts for all skeletons
        db.execute('''
            INSERT INTO skeleton_summary (skeleton_id, treenode_count) (
                SELECT treenode.skeleton_id,
                        COUNT(treenode.id) as treenode_count
                    FROM treenode
                    GROUP BY treenode.skeleton_id);
            ''')
        # Create trigger for updating treenode counts from treenode statements
        db.execute('''
            CREATE FUNCTION skeleton_summary_upsert(
                    skid skeleton_summary.skeleton_id%%TYPE)
                RETURNS void
                AS $$
                    BEGIN
                        LOOP
                            UPDATE skeleton_summary
                                SET treenode_count = treenode_count + 1
                                WHERE skeleton_id = skid;
                            IF found THEN
                                RETURN;
                            END IF;
                            BEGIN
                                INSERT INTO skeleton_summary
                                    (skeleton_id, treenode_count) VALUES
                                    (skid, 1);
                                RETURN;
            ...
            ''')
        db.execute('''
            CREATE FUNCTION treenode_update_skeleton_summary_count()
                RETURNS TRIGGER
                AS $$
                    BEGIN
                        IF (TG_OP = 'INSERT') THEN
                            -- Upsert into skeleton_summary in case this node
                            -- belongs to a new skeleton.
                            PERFORM skeleton_summary_upsert(NEW.skeleton_id);
                            RETURN NEW;
                        ELSIF (TG_OP = 'UPDATE' AND
                                OLD.skeleton_id <> NEW.skeleton_id) THEN
                            UPDATE skeleton_summary
                                SET treenode_count = treenode_count - 1
                                WHERE skeleton_id = OLD.skeleton_id;
                            -- Upsert into skeleton_summary in case this node
                            -- belongs to a newly split skeleton.
                            PERFORM skeleton_summary_upsert(NEW.skeleton_id);
                            RETURN NEW;
                        ELSIF (TG_OP = 'UPDATE') THEN
                            -- Tree joins update the nodes of both skeletons,
                            -- so when no-op ROW UPDATES happen the new row must
                            -- still be returned to prevent a trigger error in
                            -- Postgres.
                            RETURN NEW;
            ...
            ''')
        db.execute('''
            CREATE TRIGGER treenode_update_skeleton_summary_count
                AFTER INSERT OR UPDATE OF skeleton_id OR DELETE ON treenode
                FOR EACH ROW EXECUTE PROCEDURE
                    treenode_update_skeleton_summary_count();
            ''')
        db.send_create_signal(u'catmaid', ['SkeletonSummary'])

    def backwards(self, orm):
        db.execute('''
            DROP FUNCTION treenode_update_skeleton_summary_count() CASCADE;
            ''')
        db.execute('''
            DROP FUNCTION skeleton_summary_upsert(
                    skid skeleton_summary.skeleton_id%%TYPE) CASCADE;
            ''')
        # Deleting model 'SkeletonSummary'
        db.delete_table('skeleton_summary')
    ...
    models = {
        ...
        u'catmaid.skeletonsummary': {
            'Meta': {'object_name': 'SkeletonSummary', 'db_table': "'skeleton_summary'"},
            'skeleton': ('django.db.models.fields.related.OneToOneField', [], {'to': u"orm['catmaid.ClassInstance']", 'unique': 'True', 'primary_key': 'True'}),
            'treenode_count': ('django.db.models.fields.BigIntegerField', [], {})
        },
        u'catmaid.stack': {
        ...
        u'catmaid.userprofile': {
            ...
            'color': ('catmaid.fields.RGBAField', [], {'default': '(1.0, 0.9376424911155928, 0.9831742375069742, 1)'}),
